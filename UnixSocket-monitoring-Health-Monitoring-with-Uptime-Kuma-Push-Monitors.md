...
2. Click the **“+ Add New Monitor”** button on the left.
3. Fill in:
   - **Monitor Type:** `Push`
...
> 🔑 **Important:** Do **not** invent the token. Always copy the exact URL and token generated by Uptime Kuma.

### 3.2. Turn Off “Upside Down Mode”

To keep the logic simple and intuitive:

- `status=up` → service is healthy.
- `status=down` → service has a problem.

For that:

1. Click the monitor → **Edit**.
2. Make sure **“Upside Down Mode”** is **OFF**.
3. Click **Save**.
...
4. Go back to your Push monitor (**Edit → Notifications** tab).
5. Attach the notification to this monitor:
   - Enable “Alert on DOWN”.
   - Optionally enable “Alert on UP”.
6. Save.
...
> ✅ You can reuse the same `TOKEN` for multiple servers if you want a **single monitor for a group of servers**, or create one monitor per server (recommended for visibility) and use a different token on each.
...
- In your chat channel:
  - You should receive a notification **only if** the state changed from UP → DOWN or DOWN → UP.
...
2. **Simulate failure (DOWN)**
   ...
   - You should see:
     ...
     - Your chat channel receives a **“went down”** notification with `supervisord_not_activated`.
...
3. **Recovery (UP again)**
   ...
   - You should see:
     ...
     - Your chat channel receives a **“back online”** notification with `supervisord_activated`.
